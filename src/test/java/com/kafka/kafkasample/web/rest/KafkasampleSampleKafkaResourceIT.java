package com.kafka.kafkasample.web.rest;
...
import com.kafka.kafkasample.IntegrationTest;
import com.kafka.kafkasample.config.EmbeddedKafka;
import com.kafka.kafkasample.config.KafkaSseConsumer;
import com.kafka.kafkasample.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class KafkasampleSampleKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/kafkasample-sample-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/kafkasample-sample-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/kafkasample-sample-kafka/unregister"));
                return;
            }
        }
